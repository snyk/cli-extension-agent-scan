...
import (
	...
	"github.com/snyk/cli-extension-agent-scan/pkg/agentscan/errors"
	"github.com/snyk/cli-extension-agent-scan/pkg/agentscan/helpers"
	...
	"github.com/snyk/cli-extension-agent-scan/pkg/agentscan/runner"
...
func checksumForCurrentPlatform() (string, error) {
	...
	case "linux":
		if runtime.GOARCH == "amd64" {
			if AgentScanBinaryChecksumLinuxAmd64 == "" {
...
func Workflow(ctx workflow.InvocationContext, _ []workflow.Data) ([]workflow.Data, error) {
	...
	if isHelp {
		exitCode, err := runner.ExecuteBinary(ctx, []string{"help"}, AgentScanBinaryVersion, checksum, nil)
		if err != nil {
	...
	caData, err := proxy.InitCA(config, AgentScanBinaryVersion, logger)
	if err != nil {
		logger.Error().Err(err).Msg("Failed to initialize proxy CA")
		return nil, fmt.Errorf("failed to initialize proxy CA: %w", err)
	}

	wrapperProxy, err := proxy.NewWrapperProxy(config, AgentScanBinaryVersion, logger, *caData)
	if err != nil {
